...
namespace SvgViewer
{
    internal class SvgAdornment : Image
    {
        ...
        public SvgAdornment(IWpfTextView view, ITextDocumentFactoryService textDocumentFactoryService)
        {
            ...
            if (startListen == null)
            {
                ConfigurationFileReader.GetConfigurationByComponentFilePath(filePath)
                             .Match(config =>
                                    {
                                        var outputJsFilePath = config?.OutputJsFilePath;
        ...
        async Task GenerateImageAsync()
        {
            ...
            try
            {
                ...
                if (arr.IsFail)
                {
                    Trace(string.Join(Environment.NewLine, arr.Exceptions));
                    return;
                }
                ...
                UpdateUI(() =>
                {
                    ...
                    Source = ByteImageConverter.ByteToImage(arr.Value);
