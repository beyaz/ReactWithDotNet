...
namespace ___Module___
{
    /// <summary>
    /// </summary>
    delegate Task<Response<byte[]>> TakeScreenShotFunc(string url);
    ...
    static class UrlScreenShotTaker
    {
        ...
        public static async Task<TakeScreenShotFunc> GetUrlScreenShotTakerFuncAsync()
        {
            ...
            async Task<Response<byte[]>> takeScreenShot(string filePath)
            {

                trace("Taking screenshot started.");

                async Task<byte[]> take(ConfigurationContract config)
                {
                    var url = config.HtmlFilePath;
                    ...
                    var htmlElementId =  config.HtmlElementId;

                    await page.EvaluateExpressionAsync<int>("ReactDotNet.ReactElementPreview.RenderPreview('" + reactComponentTypeName + $"', '{htmlElementId}')");

                    var height = await page.EvaluateExpressionAsync<int>($"document.getElementById('{htmlElementId}').offsetHeight");
                    var width  = await page.EvaluateExpressionAsync<int>($"document.getElementById('{htmlElementId}').offsetWidth");
                ...
                return await ConfigurationFileReader.GetConfigurationByComponentFilePath(filePath).Bind(take);
