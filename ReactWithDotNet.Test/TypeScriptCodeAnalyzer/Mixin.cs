...
static class Mixin
{
    ...
    public static bool StartsWith(this IReadOnlyList<Token> tokens, string value)
    {
        ...
        if (TsAst.FindMatch(tokens, 0, valueAsTokens).isFound)
        {
            return true;
        }
        ...
        if (tokens.Count > 1)
        {
            ...
            if (reactNode.hasRead)
            {
                ...
                if (valueAsTokens.Count == tokens.Count)
                {
                    ...
                    {
                        if (!TsAst.Equals(valueAsTokens[i], tokens[tokens.Count - valueAsTokens.Count + i]))
                        {
                            return false;
                        }
                    }
