...
class MainPageFooter : ReactPureComponent
{
    ...
    {
        var styleForLinks = DisplayFlex + AlignItemsCenter;
        
        return new footer(PositionFixed, Bottom(0),WidthMaximized, BoxShadow($"{Theme[Context].grey_100} -1px -1px 1px"), DisplayFlex, JustifyContentCenter)
        {
            new MainContentContainer(FlexDirectionRow,  PaddingTopBottom(10), JustifyContentSpaceBetween, WidthMaximized, AlignItemsCenter)
            {
                new div("Copyright © 2023 Material UI SAS."),
                new FlexRow(Gap(15),AlignItemsCenter)
                {
                    // github
        ...
        {
            return new svg(WidthHeight(30), svg.Fill("rgba(0,0,0,0.54)"))
            {
                new path { d = d }
            };
        }
...
class MainPageHeader: ReactPureComponent
{
    protected override Element render()
    {
        return new header(PositionSticky, DisplayFlex, JustifyContentCenter, BoxShadow($"inset 0px -1px 1px {Theme[Context].grey_100}"))
        {
